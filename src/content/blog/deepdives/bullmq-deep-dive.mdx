...
<StateMachine
  title="BullMQ Job State Machine"
  states={[
    { id: "waiting", label: "waiting", description: "Job is in the queue ready to be claimed by a worker. Entered immediately after add(), or after delayed timer fires." },
    { id: "active", label: "active", description: "A worker has claimed the job and the processor function is executing. A lock key is held in Redis with a TTL." },
    { id: "completed", label: "completed", type: "terminal", description: "Processor resolved successfully. Job is retained in the completed sorted set for observability (default: last 1,000)." },
    { id: "failed", label: "failed", type: "error", description: "Max retry attempts exhausted. Job stays here with full data and stack trace — your dead letter store." },
    { id: "delayed", label: "delayed", description: "Job is scheduled for the future, or waiting out a backoff period before retry. Stored in a sorted set keyed by run timestamp." },
    { id: "stalled", label: "stalled", description: "Active job whose lock TTL expired — worker crashed, froze, or was killed. Detected on periodic sweep and moved back to waiting." }
  ]}
  transitions={[
    { from: "waiting", to: "active", label: "worker claims (atomic Lua)" },
    { from: "active", to: "completed", label: "processor resolves" },
    { from: "active", to: "delayed", label: "error + attempts remain (backoff)" },
    { from: "active", to: "failed", label: "max attempts exhausted" },
    { from: "active", to: "stalled", label: "lock TTL expires" },
    { from: "stalled", to: "waiting", label: "scheduler detects stall" },
    { from: "delayed", to: "waiting", label: "scheduled time passes" }
  ]}
/>
...
<Callout type="warning" title="Idempotency is not optional">
...
<Callout type="warning" title="Thundering herd on bulk add">
...
<Steps>
  ...
  <Step title="Exponential backoff with jitter">
...
<Callout type="danger" title="No queue guarantees exactly-once delivery">
...
<Comparison
  headers={["", "BullMQ", "pg-boss", "Amazon SQS", "RabbitMQ", "Apache Kafka"]}
  rows={[
    ...
    ["**Message retention**", "Redis TTL", "Configurable", "4&ndash;14 days", "Until consumed", "Configurable (days/weeks)"],
  ...
  highlight={1}
  caption="Feature comparison across major job queue and messaging systems. Column 1 (BullMQ) is highlighted."
/>
...
<Lessons type="reconsider">
  <Lesson title="Redis as a single point of failure">
  ...
  <Lesson title="Memory as the limiting resource">
  ...
  <Lesson title="Long-running jobs and lock TTLs">
  ...
  <Lesson title="No native multi-queue transactions">
  ...
  <Lesson title="Observability depth">

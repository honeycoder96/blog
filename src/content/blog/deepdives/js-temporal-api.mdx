...
Before we dig in — there's a fantastic little quiz site called **<a href="https://jsdate.wtf/" target="_blank" rel="noopener noreferrer">jsdate.wtf</a>** that fires 20 multiple-choice questions at you about the `Date` API's most absurd edge cases. It takes about 5 minutes.
...
<Callout type="danger" title="Date parsing behavior is implementation-defined">
The same conceptual date can produce two different timestamps depending on the separator you use. This is implementation-defined behavior — results vary across browsers and Node.js versions. The spec essentially says "good luck."
...
<BrowserSupport
  ...
  data={[
    { browser: "chrome", version: "144+", status: "full", note: "Shipped January 2026" },
    { browser: "firefox", version: "139+", status: "full", note: "Shipped May 2025" },
    { browser: "edge", version: "144+", status: "full" },
    { browser: "safari", version: "", status: "none", note: "Not yet implemented" },
    { browser: "node", version: "v26 tentative", status: "none", note: "Not yet — use polyfill" }
  ]}
/>

<Callout type="warning" title="Safari absence means you need a polyfill today">
...
<Tabs client:load>
  <Tab label="Named import">
    ```bash
    npm install temporal-polyfill
    ```

    ```js
    // Works alongside a future native Temporal — preferred for libraries
    import { Temporal } from 'temporal-polyfill'
    ```
  </Tab>
  <Tab label="Global polyfill">
    ```bash
    npm install temporal-polyfill
    ```

    ```js
    // Adds Temporal to globalThis — convenient for app-level usage
...
<Comparison
  ...
  caption="Common Date operations and their Temporal equivalents."
/>
...
The cookbook at <a href="https://tc39.es/proposal-temporal/docs/cookbook.html" target="_blank" rel="noopener noreferrer">tc39.es/proposal-temporal/docs/cookbook.html</a> and the <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Temporal" target="_blank" rel="noopener noreferrer">MDN Temporal docs</a> are excellent for going deeper — they have worked examples for dozens of real-world patterns from "convert between timezones" to "next occurrence of a weekday."

...
<Comparison
  headers={["", "Pessimistic Locking", "Operational Transformation (OT)", "CRDTs"]}
  rows={[
  ...
  highlight={3}
  caption="Three approaches to concurrent writes in distributed systems. CRDTs column highlighted."
/>
...
<Steps>
  <Step title="Commutativity: A ⊔ B = B ⊔ A">
  ...
  <Step title="Associativity: (A ⊔ B) ⊔ C = A ⊔ (B ⊔ C)">
...
<Callout type="info" title="Monotonicity means no true deletion">
This has an important implication: **you can never undo an operation in a state-based CRDT**. Deletion requires a workaround (tombstones, which we will cover in detail). This is not a bug — it is the source of the guarantee. Information only accumulates; it never disappears. If your system needs true deletion with guaranteed garbage collection, plan for that complexity from the start.
</Callout>
...
<Callout type="warning" title="The tombstone growth problem">
WOOT never physically removes deleted characters — it only marks them as deleted. A document that has had 100,000 characters typed and deleted has 100,000 tombstones forever. For short sessions this is fine; for documents edited over years, it becomes a serious storage and performance problem. All sequence CRDTs share this challenge to some degree — tombstone compaction requires knowing that all replicas have integrated the deletion.
</Callout>

...
<ArchFlow
  ...
  direction="vertical"
  nodes={[
    { label: "Rules Engine", sublabel: "context → template name" },
    { label: "Template Registry", sublabel: "template name → field config map" },
    { label: "Form SDK", sublabel: "field config → rendered UI" },
  ]}
/>
...
<Callout type="warning" title="Invisible fields must clear their data">
When a field becomes invisible, its data must be cleared. Without this rule, a recruiter who fills a conditional field, then changes the parent selection to hide it, will silently submit the stale value even though they never saw the field again.
</Callout>
...
<Lessons title="Engineering Lessons" type="learned">
  <Lesson title="Purity is not just an FP virtue — it's a stability contract">
    Visibility cycles (Field A depends on Field B, B depends on A) are impossible when `isVisible` functions are pure readers that cannot call anything that mutates `formData`. The renderer skips re-evaluation when visibility results are unchanged. In practice the domain model doesn't produce natural cycles, but the purity constraint prevents runaway re-renders even from buggy functions.
  </Lesson>
  <Lesson title="Keeping decision logic pure means QA is just data">
    With 100+ templates across 6 segments, a UI-driven test suite would be slow and brittle. The rules engine is a pure function: `resolveTemplate(context) → templateName`. Unit tests are fixture files — context objects and expected template names. Running the full suite is fast, offline, and gives immediate signal when a rule change produces an unexpected resolution.
  </Lesson>
  <Lesson title="Anything reading global mutable state must be late-binding">
    Rules evaluated once at page load and cached might reflect stale feature flag state, especially when flag values hydrate asynchronously after the initial render. Rules that include a `customFunction` predicate are never cached — they re-evaluate whenever context changes. Standard rules (pure data comparisons) are stable and can be memoized safely.
  </Lesson>
  <Lesson title="Configuration branching beats code branching every time">
    Desktop and PWA forms have different layout constraints but identical business logic. Field descriptors support a `versionProps` object — a partial override the SDK applies in the specific version context. The base descriptor covers all shared properties; `versionProps` patches only what differs. One template config serves both platforms with no code fork.
  </Lesson>
</Lessons>

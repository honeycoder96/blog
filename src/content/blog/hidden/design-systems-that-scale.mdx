...
The mistake most systems make: flat token files with semantic names that don't compose. The composable approach uses three layers instead.

<Tabs client:load>
  <Tab label="Flat (avoid)">
    ```css
    /* Don't do this */
...
The systems that fail: those that prioritize completeness over usability. Engineers adopt what they trust.

<Callout type="tip" title="10 excellent components beats 50 mediocre ones">
Ship 10 excellent, well-documented components instead of 50 mediocre ones. The systems that die are the ones that prioritize coverage over quality — 50 components nobody trusts is worse than 10 components everyone reaches for first.
</Callout>

...
| Header | What it says |
|--------|-------------|
| `Sec-Fetch-Site` | Relationship between requester and target: `same-origin`, `same-site`, `cross-site`, or `none` (user navigation) |
...
A request from DevForum's own JavaScript to its own API:
```http
...
function resourceIsolationPolicy(req, res, next) {
  ...
  if (!site || site === 'same-origin') return next();
  ...
  // (site header absent — fall through to token validation)
  if (!site) return next();
...
<Lessons title="Cookie Security Checklist" type="recommendations">
  ...
  <Lesson title="Never use Path= as a security boundary">

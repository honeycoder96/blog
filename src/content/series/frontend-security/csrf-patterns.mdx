...
<SequenceDiagram
  title="Double-Submit Cookie — How the Same Value Proves Same Origin"
  actors={[
    { id: "server",  label: "DevForum Server", icon: "server", variant: "trusted" },
    { id: "browser", label: "Legitimate Browser", icon: "browser" },
    { id: "evil",    label: "evil.com", variant: "malicious" }
  ]}
  messages={[
    { from: "server",  to: "browser", label: "Login response: Set-Cookie: csrfToken=abc (httpOnly: false)", type: "response", annotation: "JS-readable cookie" },
    { from: "browser", to: "browser", label: "JS reads csrfToken cookie → adds X-CSRF-Token: abc header", type: "note" },
    { from: "browser", to: "server",  label: "POST /account/settings", sublabel: "Cookie: csrfToken=abc | X-CSRF-Token: abc", type: "request", highlight: true },
    { from: "server",  to: "browser", label: "Header matches cookie — 200 OK", type: "response", annotation: "values match → legitimate" },
    { from: "evil",    to: "server",  label: "POST /account/settings (forged)", sublabel: "Cookie: csrfToken=abc (auto-sent) | X-CSRF-Token: ??? (cannot read cookie)", type: "attack" },
    { from: "server",  to: "evil",    label: "Header missing or wrong — 403 Forbidden", type: "blocked", annotation: "SOP blocks evil.com from reading the cookie" }
  ]}
  caption="The attacker's browser sends the csrfToken cookie automatically — but evil.com's JavaScript cannot read it (SOP). Without the cookie's value, the attacker cannot forge a matching X-CSRF-Token header. The mismatch triggers a 403."
/>
...
<Callout type="warning" title="Where you store your token matters as much as which CSRF pattern you use">
  **Cookies** (with `HttpOnly`, `Secure`, `SameSite=Strict`): protected from JavaScript reads; auto-sent by the browser but `SameSite` blocks cross-site requests. The right default for session tokens.

  **`localStorage`**: readable by any JavaScript on the page — so XSS immediately yields the token. Not auto-sent, so it survives CSRF from the browser side. But XSS grants the same access anyway. Trading one attack class for another is not a gain.

  The practical rule: **store session tokens in cookies, never in `localStorage`**. CSRF protection (this post) and XSS protection (Post 1) together cover both attack vectors. Storing tokens in `localStorage` to "avoid CSRF" removes the XSS barrier and leaves you defenceless against the more common threat.
</Callout>
